...
# ─────────────
# 2-1. DP multi GPU train
# ─────────────
def _unwrap_model(model):
    parallel_types = (torch.nn.DataParallel, DDP)
...
def _parallelize_model(model, gpu_ids=None):
    ...
    print(f"DataParallel 사용: GPU {gpu_ids}")
    return torch.nn.DataParallel(model, device_ids=gpu_ids, output_device=gpu_ids[0])


def _mean_parallel_loss(loss):
    return loss.mean() if loss.dim() > 0 else loss


def train_chunk_parallel(model_type = 'hes', barr_type='barr', dim_z=8, hidden_dims=None, batch_size=1024,
                         n_epochs=200, lr=1e-3, beta=1.0, save_path='cvae.pt',
                         gpu_ids=None):
    ...
    if os.path.exists(save_path):
        ...
        if choice == '1':
            ...
            _load_state_dict(cvae, load_model['model_state'])
            cvae.to(device)
            cvae.eval()
            print("모델 불러오기 완료")
            return cvae, load_model['loss_history'], load_model['eta_min'], load_model['eta_max']
    ...
    # 학습
    cvae = _parallelize_model(cvae, gpu_ids=gpu_ids)
    optimizer    = torch.optim.Adam(cvae.parameters(), lr=lr)
    scheduler    = torch.optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.5)
    loss_history = {'recon_loss': [], 'KL_loss': [], 'total_loss': []}
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        def train_one_chunk(dataset):
            ...
            for x_batch, eta_batch in dataloader:
                ...
                recon_loss, kl_loss = cvae(x_batch, eta_batch) # 동기식 병렬. DataParallel로 감싼 객체를 호출할 때만 병렬 처리.
                recon_loss = _mean_parallel_loss(recon_loss)
                kl_loss    = _mean_parallel_loss(kl_loss)
                loss = recon_loss + beta * kl_loss
        ...
        if epoch % 10 == 0:
            torch.save({
                'model_state' : _unwrap_model(cvae).state_dict(), # model weight
                'eta_min'     : eta_min,
                'eta_max'     : eta_max,
                'dim_x'       : dim_x,
                'dim_eta'     : dim_eta,
    ...
    total_time = time.perf_counter() - train_start
    torch.save({
        'model_state' : _unwrap_model(cvae).state_dict(),
        'eta_min'     : eta_min,
        'eta_max'     : eta_max,
        'dim_x'       : dim_x,
        'dim_eta'     : dim_eta,
    ...
    return _unwrap_model(cvae), loss_history, eta_min, eta_max


# ─────────────
# 2-2. DDP multi GPU train
# ─────────────
def _ddp_setup():
