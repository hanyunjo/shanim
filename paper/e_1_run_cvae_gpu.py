...
class ChunkDataset(Dataset):
    # 청크 파일 하나를 RAM에 로드. 기존 benchmark/fallback용으로 남겨둔다.
...
class GpuChunk:
    # 청크 파일 하나를 GPU tensor로 보관한다. DataLoader 없이 직접 slicing해서 학습한다.
...
def train_chunk(model_type = 'hes', dim_z=8, hidden_dims=None, batch_size=1024,
    ...
    def load_chunk_gpu(ci):
        chunk_path = chunk_path_by_idx[int(ci)]
        with h5py.File(chunk_path, 'r') as f:
            paths = f['paths'][:] # (N, 3) : [ori_idx, X_T, M_T]
    ...
    def train_one_chunk(dataset, global_chunk, shuffle_rows=True):
        cvae.train()
    ...
    with ThreadPoolExecutor(max_workers=1) as prefetcher:
        ...
        future = prefetcher.submit(load_chunk_gpu, first_ci)
        ...
        for offset in range(num_chunks):
            ...
            if offset + 1 < num_chunks:
                ...
                future = prefetcher.submit(load_chunk_gpu, next_ci)
